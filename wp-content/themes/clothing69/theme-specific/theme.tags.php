...
if ( !function_exists('clothing69_need_page_title') ) {
	function clothing69_need_page_title() {
		return !is_front_page() && apply_filters('clothing69_filter_need_page_title', true);
	}
}
...
if ( !function_exists('clothing69_show_layout') ) {
	function clothing69_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('clothing69_get_logo_image') ) {
	function clothing69_get_logo_image($type='') {
		$logo_image = '';
		if (clothing69_get_retina_multiplier(2) > 1)
			$logo_image = clothing69_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = clothing69_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('clothing69_get_header_video') ) {
	function clothing69_get_header_video() {
		$video = '';
		if (apply_filters('clothing69_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (clothing69_check_theme_option('header_video')) {
				$video = clothing69_get_theme_option('header_video');
...
if ( !function_exists('clothing69_show_post_meta') ) {
	function clothing69_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			if ( !empty($args['categories'])) {
				$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('clothing69_filter_get_post_categories', '');
				if (!empty($cats)) {
					?>
					<span class="post_meta_item post_categories"><?php clothing69_show_layout($cats); ?></span>
					<?php
				}
			}
			// Post date
			if ( !empty($args['date'])) {
				$dt = apply_filters('clothing69_filter_get_post_date', clothing69_get_date());
				if (!empty($dt)) {
					?>
			...
			// Post counters
			clothing69_show_layout(clothing69_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				clothing69_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'clothing69'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'clothing69' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('clothing69_show_post_featured') ) {
	function clothing69_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => clothing69_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => clothing69_is_on(clothing69_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : clothing69_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? clothing69_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = clothing69_get_post_image();
				if (!empty($image)) 
					$image = clothing69_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . clothing69_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="http://schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$clothing69_attr = clothing69_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($clothing69_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($clothing69_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						clothing69_hovers_add_icons($args['hover']);
					}
					...
					if (($output=clothing69_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						clothing69_show_layout($output);
					...
					if (!empty($image)) {
						...
						clothing69_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if ($post_format == 'video' && !$args['thumb_only']) {
					$video = clothing69_get_post_video('', false);
					if (empty($video))
							$video = clothing69_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = clothing69_array_get_first($videos, false);
								}
							}
						}
					if (!empty($video)) {
						if ( $has_thumb ) {
							$video = clothing69_make_video_autoplay($video);
							?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
						}
						?><div class="post_video video_frame"><?php 
							if ( !$has_thumb ) {
								clothing69_show_layout($video);
							}
						?></div><?php
					}
				...
				// Put audio over the thumb
						...
						$audio = clothing69_get_post_audio($post_content, false);
					if (empty($audio))
							$audio = clothing69_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = clothing69_array_get_first($audios, false);
							}
							}
						}
						if (!empty($audio)) {
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = clothing69_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = clothing69_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !clothing69_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php clothing69_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !clothing69_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php clothing69_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							clothing69_show_layout($audio); 
						?></div><?php
					}
				}
				}
				...
				clothing69_show_layout($post_info);
				?></div><?php
			} else {
				// Put optional info block over the thumb
				clothing69_show_layout($post_info);
...
if ( !function_exists('clothing69_get_no_image') ) {
	function clothing69_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = clothing69_get_theme_option( 'no_image' );
			if (empty($img)) $img = clothing69_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('clothing69_add_bg_in_post_nav') ) {
	function clothing69_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = clothing69_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), clothing69_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), clothing69_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'clothing69-main', $css );
	}
}

// Show related posts
if ( !function_exists('clothing69_show_related_posts') ) {
	function clothing69_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('RELATED POSTS', 'clothing69');
				?></h3>
...
if ( !function_exists('clothing69_show_portfolio_posts') ) {
	function clothing69_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? clothing69_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = clothing69_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = clothing69_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the clothing69_ajax_get_posts action
if ( !function_exists( 'clothing69_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_clothing69_ajax_get_posts',			'clothing69_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_clothing69_ajax_get_posts',	'clothing69_ajax_get_posts_callback');
	function clothing69_ajax_get_posts_callback() {
		...
		if ((int) $id > 0) {
			...
			clothing69_storage_set('blog_mode', 'blog');
			clothing69_storage_set('options_meta', get_post_meta($id, 'clothing69_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => clothing69_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'clothing69');
		}
...
// Show pagination
if ( !function_exists('clothing69_show_pagination') ) {
	function clothing69_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = clothing69_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'clothing69' ),
				'next_text' => esc_html__( '>', 'clothing69' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'clothing69' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'clothing69'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'clothing69') : esc_html__('Newest posts', 'clothing69') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'clothing69') : esc_html__('Older posts', 'clothing69'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}

...
if (!function_exists('clothing69_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'clothing69_hovers_theme_setup3', 3 );
	function clothing69_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		clothing69_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'clothing69'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'clothing69') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'clothing69'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'clothing69'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'clothing69') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'clothing69'),
					'icon'	=> esc_html__('Icon',	'clothing69'),
					'icons'	=> esc_html__('Icons',	'clothing69'),
					'zoom'	=> esc_html__('Zoom',	'clothing69'),
					'fade'	=> esc_html__('Fade',	'clothing69'),
					'slide'	=> esc_html__('Slide',	'clothing69'),
					'pull'	=> esc_html__('Pull',	'clothing69'),
					'border'=> esc_html__('Border',	'clothing69')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('clothing69_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'clothing69_hovers_theme_setup9', 9 );
	function clothing69_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'clothing69_hovers_frontend_scripts', 1010 );
		add_filter( 'clothing69_filter_localize_script','clothing69_hovers_localize_script' );
		add_filter( 'clothing69_filter_merge_scripts',	'clothing69_hovers_merge_scripts' );
		add_filter( 'clothing69_filter_merge_styles',	'clothing69_hovers_merge_styles' );
		add_filter( 'clothing69_filter_get_css', 		'clothing69_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'clothing69_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'clothing69_hovers_frontend_scripts', 1010 );
	function clothing69_hovers_frontend_scripts() {
		if ( clothing69_is_on(clothing69_get_theme_option('debug_mode')) && clothing69_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'clothing69-hovers', clothing69_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( clothing69_is_on(clothing69_get_theme_option('debug_mode')) && clothing69_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'clothing69-hovers',  clothing69_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('clothing69_hovers_merge_scripts')) {
	//Handler of the add_filter( 'clothing69_filter_merge_scripts', 'clothing69_hovers_merge_scripts' );
	function clothing69_hovers_merge_scripts($list) {
...
if (!function_exists('clothing69_hovers_merge_styles')) {
	//Handler of the add_filter( 'clothing69_filter_merge_styles', 'clothing69_hovers_merge_styles' );
	function clothing69_hovers_merge_styles($list) {
...
if (!function_exists('clothing69_hovers_localize_script')) {
	//Handler of the add_filter( 'clothing69_filter_localize_script','clothing69_hovers_localize_script' );
	function clothing69_hovers_localize_script($arr) {
		$arr['button_hover'] = clothing69_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('clothing69_hovers_add_icons') ) {
	function clothing69_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'clothing69') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'clothing69');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'clothing69_hovers_get_css' ) ) {
	//Handler of the add_filter( 'clothing69_filter_get_css', 'clothing69_hovers_get_css', 10, 4 );
	function clothing69_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.sc_button_hover_style_extra.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['extra_dark']} 50%, {$colors['extra_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_extra.sc_button_hover_slide_right {		background: linear-gradient(to left,	{$colors['extra_dark']} 50%, {$colors['extra_link']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_link']} !important; }
.sc_button_hover_style_extra.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['extra_dark']} 50%, {$colors['extra_link']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_link']} !important; }
.sc_button_hover_style_extra.sc_button_hover_slide_bottom {		background: linear-gradient(to top,		{$colors['extra_dark']} 50%, {$colors['extra_link']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_link']} !important; }
...
.sc_button_hover_style_extra.sc_button_hover_slide_bottom  {	color: {$colors['bg_color']} !important; }

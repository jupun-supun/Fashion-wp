...
add_filter( 'clothing69_filter_sidebar_present', 'clothing69_collection_sidebar_present' );
function clothing69_collection_sidebar_present($present) {
	return false;
}
...
while ( have_posts() ) { the_post();
	...
	if ( is_single() && !is_attachment() ) {
		...
		<div class="post_meta post_meta_single_coll"><div class="cat_wrap"><?php
			echo esc_html_e('Category:','clothing69');
			// Post taxonomies
	...
	if(($first_title && $first_link) || ($second_title && $second_link)){
		...
		if($first_title && $first_title){?>
			<a class="first" href="<?php echo esc_url($first_link); ?>"><?php clothing69_show_layout($first_title); ?></a>
		<?php }
		if($second_title && $second_link){?>
			<a class="second" href="<?php echo esc_url($second_link); ?>"><?php echo clothing69_show_layout($second_title); ?></a>
		<?php }
		echo '</div>';
	}

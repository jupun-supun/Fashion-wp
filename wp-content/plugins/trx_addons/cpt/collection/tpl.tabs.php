<?php
/**
 * The style "tabs" of the collection
 *
 * @package WordPress
...
if ((int)$query->found_posts > 0) {
	...
	?><div class="sc_collection sc_collection_<?php
		...
		?><div class="sc_collection_content sc_item_content"><?php
			...
			?><div class="sc_collection_tabs_content">
				<?php
				$tabs_list = array();
				...
				while ( $query->have_posts() ) { $query->the_post();
					...
					$tabs_item = '<div class="sc_collection_tabs_list_item' . ($trx_addons_number-1 == $args['offset'] ? ' sc_collection_tabs_list_item_active' : '') . '">';
					$tabs_add = '';
					if ($args['featured']=='icon') {
						...
						$tabs_add = sprintf('<span class="sc_collection_item_number">%02d</span>', $number);
					}
					$tabs_item .= '<h6 class="sc_collection_item_title' . ($tabs_add ? ' with_icon' : '') . '">'
										. $tabs_add 
										. '<span class="sc_collection_item_label">' . get_the_title() . '</span>'
										. '</h6>';
					$tabs_list[] = $tabs_item . '</div>';
					trx_addons_get_template_part('cpt/collection/tpl.tabs-item.php');
				}
				wp_reset_postdata();
				?>	
			</div><?php
			...
			// Tabs list
			?><div class="sc_collection_tabs_list">
				<?php

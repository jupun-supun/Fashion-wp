...
if (!function_exists('trx_addons_components_init')) {
	add_action( 'trx_addons_filter_options', 'trx_addons_components_init', 1 );
...
*/
			...
			$blocks = array(
				'cpt' => __('Custom Post Types', 'trx_addons'),
				'sc' => __('Shortcodes', 'trx_addons'),
				'widgets' => __('Widgets', 'trx_addons'),
				'components' => __('Other components', 'trx_addons'),
			);
...
if ( !function_exists( 'trx_addons_components_importer_export' ) ) {
	...
	function trx_addons_components_importer_export($importer) {
		...
		trx_addons_fpc(trx_addons_get_file_dir('importer/export/components.txt'), $output);
	}
}
